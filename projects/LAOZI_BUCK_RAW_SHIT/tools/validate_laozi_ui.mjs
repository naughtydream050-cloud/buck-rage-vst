...
if (!spec.preset.textBounds) findings.push("missing custom preset text bounds");
if (!spec.meter.left || !spec.meter.right) findings.push("missing independent meter bounds");
if (!spec.buttons.oversampleStatusBounds || !spec.buttons.bypassIndicatorBounds) findings.push("missing custom button status bounds");
const status = findings.length === 0 ? "passed" : "failed";
const report = { schemaVersion: 1, status, score: status === "passed" ? 10 : 0, visualMode: "image-first", checks: { referencePresent: fs.existsSync(path.join(root, spec.assets.reference)), faceplatePresent: fs.existsSync(path.join(root, spec.assets.faceplate)), knobCount: spec.knobs.length, circularKnobBounds: findings.filter((item) => item.includes("circular")).length === 0, customPresetOverlayBounds: !!spec.preset.textBounds, independentMeterBounds: !!spec.meter.left && !!spec.meter.right, customButtonStatusBounds: !!spec.buttons.oversampleStatusBounds && !!spec.buttons.bypassIndicatorBounds }, findings };
